...
import type { Clinic, ReviewAnalysis } from "@/lib/types"
...
export function PractitionerInsights({ clinic }: PractitionerInsightsProps) {
  const reviewAnalysis: ReviewAnalysis = clinic.reviewAnalysis
  

  if (!reviewAnalysis) return null

  const practitioners = reviewAnalysis.practitioners

  return (
    <div className="space-y-6">
      {/* Practitioner Attributes */}
      <Tabs defaultValue={practitioners[0]?.name || ""} className="w-full">
      {/* Tab Headers */}
      <div className="flex justify-center">
  <TabsList className="flex flex-wrap gap-2">
    {practitioners.map((p, idx) => (
      <TabsTrigger key={idx} value={p.name}>
        {p.name}
      </TabsTrigger>
    ))}
  </TabsList>
      ...
      {/* Tab Content */}
      {practitioners.map((p, idx) => (
        <TabsContent key={idx} value={p.name} className="mt-6">
          <Card className="border-border/50">
            ...
            <CardContent className="space-y-6">
              ...
              {p.interpersonal_skills?.length > 0 && (
                <div>
                  <h4 className="font-medium text-foreground mb-3">
                    Interpersonal Skills
                  </h4>
                  <div className="flex flex-wrap gap-2">
                    {p.interpersonal_skills.map((skill, i) => (
