...
export default function ClinicEditor() {
  const [clinic, setClinic] = useState<Partial<Practitioner>>({})
  const [keys, setKeys] = useState<(keyof Practitioner)[]>([])
  ...
  useEffect(() => {
    ...
    fetch(`/directory/api/admin/practitioners/${params.slug}`)
      .then(res => res.json())
      .then(setClinic)
  ...
  setKeys(Object.keys(clinic) as (keyof Practitioner)[])
}, [clinic])
  

  const handleChange = (field: keyof Practitioner, value: any) => {
    setClinic({ ...clinic, [field]: value })
  }

  const handleSubmit = async () => {
    const url = isNew ? '/api/admin/practitioners' : `/api/admin/practitioners/${params.slug}`
